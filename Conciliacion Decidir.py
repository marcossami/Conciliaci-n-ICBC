...
CANAL = st.radio("Elegí el canal a conciliar", ["ICBC Mall", "Carrefour"], horizontal=True)
...
def find_col(cols, pistas_any):
    cols_low = [c.strip().lower() for c in cols]
    for i, cl in enumerate(cols_low):
        if any(p in cl for p in pistas_any):
            return cols[i]
    return None
...
# =========================
def run_icbc():
    st.subheader("ICBC Mall — Decidir vs Aper")
...
# =========================
def run_carrefour():
    st.subheader("Carrefour Marketplace — Reporte Carrefour vs Reporte CTC")
    ...
    if file_carrefour and file_ctc:
        # Lectura básica y normalización de headers
        ...
        # =============== Selección de columnas a matchear ===============
        ...
        # Monto en cada reporte
        monto_car = st.selectbox("Columna **Monto** en Carrefour", cols_car, key="monto_car")
        monto_ctc = st.selectbox("Columna **Monto** en CTC", cols_ctc, key="monto_ctc")

        # (Opcional) Fecha en cada reporte
        ...
        df_ctc["_monto"] = normalize_money(df_ctc[monto_ctc])
        ...
        # Agrupar por ID (por si hay múltiples filas por orden)
        ...
        c2.metric("Total CTC", f"{total_ctc:,.2f}")
        c3.metric("Diferencia", f"{diff_abs:,.2f}", delta=f"{diff_total:,.2f}")
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            for sheet, df in [("Conciliados", m), ("Carrefour_sin_CTC", car_solo), ("CTC_sin_Carrefour", ctc_solo)]:
                ...
                # formateo de columnas de dinero si existen
